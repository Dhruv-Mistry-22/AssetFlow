...
export function StateProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadState = () => {
      ...
      const loadedUsers = getOrSeed('af_users', seedUsers);
      const loadedDepts = getOrSeed('af_departments', seedDepartments);
      const loadedCats = getOrSeed('af_categories', seedCategories);
      const loadedAssets = getOrSeed('af_assets', seedAssets);
      const loadedAllocs = getOrSeed('af_allocations', seedAllocations);
      const loadedTransfers = getOrSeed('af_transfers', []);
      const loadedBookings = getOrSeed('af_bookings', seedBookings);
      const loadedMaint = getOrSeed('af_maintenance', seedMaintenance);
      const loadedAudits = getOrSeed('af_audits', seedAudits);
      const loadedAuditItems = getOrSeed('af_audit_items', seedAuditItems);
      const loadedLogs = getOrSeed('af_logs', seedLogs);
      const loadedNotifs = getOrSeed('af_notifications', seedNotifications);

      setUsers(loadedUsers);
      setDepartments(loadedDepts);
      setCategories(loadedCats);
      setAssets(loadedAssets);
      setAllocations(loadedAllocs);
      setTransferRequests(loadedTransfers);
      setBookings(loadedBookings);
      setMaintenanceRequests(loadedMaint);
      setAuditCycles(loadedAudits);
      setAuditItems(loadedAuditItems);
      setLogs(loadedLogs);
      setNotifications(loadedNotifs);

      // Default user is the Admin (Aditi Rao)
      ...
      if (cachedUser) {
        ...
        setCurrentUserState(loadedUsers[0]);
        localStorage.setItem('af_current_user', JSON.stringify(loadedUsers[0]));
      }
      setInitialized(true);
    };
  ...
  const registerAsset = (data: Omit<Asset, 'id' | 'assetTag' | 'status' | 'categoryName'>) => {
  ...
  const raiseMaintenance = (data: {
    ...
    saveToStorage('af_maintenance', updatedMaint);
